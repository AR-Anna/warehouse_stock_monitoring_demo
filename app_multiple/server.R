...
shinyServer(function(input, output, session) {
    ...
    output$text <- renderUI({
        ...
        info <-
            data.frame(Seconds_Behind_Master = 0,
        ...
        str4 <-
            ifelse(str3 == "Cancellation:  NA NA:NA:NA",
                "Cancellation: ?",
                str3)
    ...
    output$title <- renderText({
        ...
        sprintf(
            "Warehouse Stock Monitoring (Update: %s)",
            format(lubridate::with_tz(fr$UPDATE_TIME, tz), format = "%Y.%m.%d. %H:%M")
        )
    })
    ...
    observeEvent(input$friss, {
        ...
        update_time <-
            data.frame(UPDATE_TIME = ymd_hms("2021-01-31 20:11:59"))
        
        update_time_plus <- update_time$UPDATE_TIME + 600
        
        if (Sys.time() <= update_time_plus) {
            shinyalert("The update has been completed.",
                type = "success",
                animation = TRUE)
        } else {
    ...
    output$pivot_device_type <- renderRpivotTable({
        ...
        alcs_df <-
            read.xlsx("C:/Users/Admin/Desktop/Github_demo/warehouse_equipments.xlsx")
        
        alcs_df2 <- alcs_df %>%
            select(year, month, equipment_type, movement, total)
        colnames(alcs_df2) <-
            c("Year", "Month", "Type", "Movement", "Piece")
        
        rpivotTable(
            alcs_df2,
            rows = c("Type", "Movement"),
            cols = c("Year", "Month"),
            vals = "Piece",
            rendererName = "Table With Subtotal Row Heatmap",
            aggregatorName = "Integer Sum" ,
            subtotals = TRUE,
            width = "100%",
            unusedAttrsVertical = F
        )
    ...
    output$dl2 <- downloadHandler(
        filename = function() {
            paste0("equipment_movement_group_type_", date, ".xlsx")
        },
        content = function(file) {
            ...
            colnames(y) <-
                c("Type", "Year", "Month", "Movement", "Piece")
        ...
        local({
            ...
            output[[nev]] <- renderPlot({
                ...
                y <- df %>%
                    ...
                    mutate(pc_value = ifelse(
                        movement == "At Costumers",
                        -1 * pc_label,
                        pc_label
                    ))
                ...
                plot_type <- function(type) {
                    ...
                    a <-
                        ifelse(
                            plot$equipment_type == "C",
                            -500,
                            ifelse(
                                plot$equipment_type == "E",
                                -50,
                                ifelse(
                                    plot$equipment_type == "H",
                                    -300,
                                    ifelse(
                                        plot$equipment_type == "valami",
                                        -100,
                                        ifelse(plot$equipment_type == "F", -300, -200)
                                    )
                                )
                            )
                        )
                    ...
                    p <- plot %>%
                        ...
                        scale_fill_manual("Movement",
                            values = c("#CFCECA", "#dac38b")) +
                        scale_x_discrete(name = "",
                            limits = rev(levels(y$month_name))) +
                        geom_text(
                            aes(
                                y = ifelse(movement == "At Costumers", a, b),
                                label = pc_value
                            ),
                            color = "black",
                            size = 4,
                    ...
                    p + theme_bw() +
                        ...
                        labs(title = paste0(plot$equipment_type[1]),
                            subtitle = "Movement of devices between customers and the warehouse")
                }
    ...
    observeEvent(input$openModal, {
        showModal(modalDialog(
            ...
            HTML(
                "The purpose of the application is to show a solution how we can monitoring movements of devices (uniqualy iditified) at a fictive telecommunication company.</br>
